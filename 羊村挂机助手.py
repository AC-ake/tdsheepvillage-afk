...
class AssistantApp:
    ...
    def build_main(self, frame: ttk.Frame) -> None:
        ...
        ttk.Label(box, text="原作脚本主体来自羊友：鹏小白就是我；整合想法来自A_c；操刀师傅ChatGPT",
                  foreground="#666").pack(anchor="w", padx=12, pady=5)         
    ...
    def start_selected(self) -> None:
        ...
        if self.tower_enabled.get():
            if not self.tower_coords: messagebox.showwarning("缺少塔坐标", "请先在“自动升塔”页面录入塔坐标，建议升塔数略大于苦工数。"); return
    ...
    def toggle_wolf(self) -> None:
        ...
        try:
            ...
            if not (IMAGE_DIR / "jiandielang.png").is_file():
                raise ValueError("缺少命名为 jiandielang.png的间谍狼图片，请放入“图像”文件夹")
